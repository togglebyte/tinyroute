...
async fn agent_to_agent() {
    ...
    agent_a.send(Address::B, message.clone());

    let msg = agent_b.recv().await.unwrap();
    assert!(matches!(msg, Message::Value(message, Address::A)));

    agent_b.shutdown_router();
    handle.await;
}

#[tokio::test]
async fn shutdown_router() {
    let (agent_a, agent_b, handle) = setup();
...
async fn shutdown_agent() {
    let (agent_a, mut agent_b, handle) = setup();
    agent_a.send_shutdown(Address::B);
